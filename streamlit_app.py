...
from main.visualization import plot_correlation_heatmap, plot_boxplots_vs_left, plot_correlation_with_target, plot_class_balance
...
def load_trained_model():
    with open("data/trained_pipeline_xgb_alldata_noreduction_2024-09-21.pkl", "rb") as file:
...
# Sidebar
st.sidebar.image("data/Novo_Nordisk_Logo.png", use_column_width=False, width=100)
#st.sidebar.title("Navigation")
page = st.sidebar.radio("", ["Exploratory Data Analysis", "Model Pipeline", "Model Evaluation", "Attrition Prediction Simulation"])
...
if page == "Exploratory Data Analysis":
    ...
    st.markdown("<h2 class='small-header'>1. Department Metrics</h2>", unsafe_allow_html=True)
    st.markdown("<p class='explanation'>Below is the analysis of various metrics by department.</p>", unsafe_allow_html=True)
    ...
    st.markdown("<h2 class='small-header'>2. Attrition Analysis</h2>", unsafe_allow_html=True)
    st.markdown("<p class='explanation'>Pie charts showing employee attrition by department and salary.</p>", unsafe_allow_html=True)
    ...
    st.markdown("<h2 class='small-header'>3. Percentage of Employees Left by Salary Category</h2>", unsafe_allow_html=True)
    percentage_employees_left_img = load_image("data/Percentage_Employees_Left.png")
    st.image(percentage_employees_left_img, caption="Percentage of Employees Left by Salary Category")

    # Display Distribution of Features
    st.markdown("<h2 class='small-header'>4. Distribution of Features</h2>", unsafe_allow_html=True)
    st.markdown("<p class='explanation'>Distribution of key numerical features such as satisfaction, evaluation, monthly hours, and time spent in the company.</p>", unsafe_allow_html=True)
    ...
    st.markdown("<h2 class='small-header'>5. Correlation Heatmap</h2>", unsafe_allow_html=True)
    ...
    st.markdown("<h2 class='small-header'>6. Correlation with Target</h2>", unsafe_allow_html=True)
    fig_plotly = get_correlation_with_target(df)
    st.plotly_chart(fig_plotly)

    # 7. Boxplots vs Left (Keep dynamic)
    st.markdown("<h2 class='small-header'>7. Boxplots vs Left</h2>", unsafe_allow_html=True)
    st.markdown("<p class='explanation'>Boxplots comparing numerical features with employee attrition.</p>", unsafe_allow_html=True)
    fig = get_boxplots_vs_left(df)
    st.pyplot(fig)

    # 8. Class Balance of Target Variable (Keep dynamic)
    st.markdown("<h2 class='small-header'>8. Class Balance of Target Variable</h2>", unsafe_allow_html=True)
...
if page == "Model Pipeline":
    ...
    st.image(pipeline_image, caption="Attrition Modeling Pipeline", use_column_width=True)
...
# Evaluation Page
    ...
    st.markdown("<p class='explanation'>Here you can evaluate the XGBoost model predictions, view the confusion matrix, ROC curve, and SHAP summary plot.</p>", unsafe_allow_html=True)

    # Add evaluation metrics
    st.markdown("<h2 class='small-header'>Model Evaluation Metrics</h2>", unsafe_allow_html=True)
    ...
    if st.button("Show SHAP Summary Plot"):
        ...
        st.markdown("<h2 class='small-header'>SHAP Plot Interpretation</h2>", unsafe_allow_html=True)
    ...
    st.markdown("<h1 class='title'>Attrition Prediction Simulation</h1>", unsafe_allow_html=True)

    # Input sliders for user to set feature values in the sidebar
    with st.sidebar:
        st.markdown("<h2 class='small-header'>Input Parameters for Prediction</h2>", unsafe_allow_html=True)

        satisfaction = st.slider('Satisfaction Level', 0.0, 1.0, step=0.01, label_visibility='visible')
        evaluation = st.slider('Last Evaluation', 0.0, 1.0, step=0.01, label_visibility='visible')
        monthly_hours = st.slider('Average Monthly Hours', 80, 320, step=1, label_visibility='visible')
        ...
        department = st.selectbox("Department", sorted([
            "sales", "accounting", "hr", "technical", "support", "management", "product_mng", "marketing"
        ]))

        work_accident = st.selectbox("Work Accident (0 = No, 1 = Yes)", [0, 1])
        promotion_last_5years = st.selectbox("Promotion in Last 5 Years (0 = No, 1 = Yes)", [0, 1])
    ...
    if st.button("Predict Attrition"):
        ...
        st.markdown(f"<p style='font-size:14px;'>**Prediction**: The employee is likely to <b>{prediction_label}</b> with a probability of {prediction_proba*100:.2f}%</p>", unsafe_allow_html=True)
        ...
        cleaned_feature_names = [name.replace('numeric__', '').replace('categorical__', '') for name in feature_names]

        # Use the cleaned feature names in SHAP plot
        if shap_values is not None:
            st.markdown("<h4 style='font-size:16px;'>SHAP Explanation:</h4>", unsafe_allow_html=True)
            #shap.initjs()
            ...
            shap.waterfall_plot(shap.Explanation(values=shap_values[0],
                                                ...
                                                feature_names=cleaned_feature_names))  # Use cleaned feature names
            st.pyplot(fig)
