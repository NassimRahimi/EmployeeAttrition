...
class BinaryModelEvaluator(ModelEvaluator):
    ...
    def build_roc_curve(input_frames=[]):
        ...
        for input_frame in input_frames:
            fpr, tpr, _ = roc_curve(y_true=input_frame[0], y_score=input_frame[1], pos_label=1)
            roc_auc = round(roc_auc_score(y_true=input_frame[0], y_score=input_frame[1]), 4)
            ax.plot(fpr, tpr, lw=lw, label=f"ROC curve {input_frame[2]} (AUC = {roc_auc})")

        ax.plot([0, 1], [0, 1], color="navy", lw=lw, linestyle="--", label="Random estimator")
    ...
    def build_feature_importances(f_importances=[], f_names=[], top=50):
        ...
        ax.barh(y_pos, importance, align="center", color="magenta", ecolor="black", alpha=0.5)
        ax.grid()
        ax.set_yticks(y_pos)
    ...
    def generate_pdf_report(self, filepath, f_importances, f_names, input_frames, y_test_true, y_test_pred, y_train_true, y_train_pred):
        pdfmetrics.registerFont(TTFont("Arial", "Arial.ttf"))

        doc = SimpleDocTemplate(filepath, pagesize=landscape(letter), rightMargin=72, leftMargin=72, topMargin=72, bottomMargin=18)

        styles = getSampleStyleSheet()
        story = []
        story.append(Paragraph("Employee Attrition Prediction - Model Evaluation", styles["Title"]))
        ...
        self.build_confusion_matrix(y_train_true, y_train_pred, ax[0], "Confusion Matrix - Train")
        self.build_confusion_matrix(y_test_true, y_test_pred, ax[1], "Confusion Matrix - Test")
...
if __name__ == "__main__":
    ...
    input_frames = [
        [y_train, y_train_hat, "Train"],
        [y_test, y_test_hat, "Test"]
    ]

    # Generate the PDF report
    evaluator = BinaryModelEvaluator()
    evaluator.generate_pdf_report(filepath=f"evaluation_report.pdf",
